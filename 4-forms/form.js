   ...
   // Array
    ...
    {	//hämtar ut id
        ...
        if (firstNameField.lastChild.hasChildNodes()) // Tar bort eventuellt utskrivet felmeddelande
        {
        	...
        	//om fältet är blankt,skriv ut felmedelande
        if (input.value.trim() == "")
        {
            ...
            firstNameField.appendChild(emptyErrorTag); // lägger in som child i firstname
    ...
    validatePostalCode: function(input)		//Postadress
    {
        var postalCoderegExp = [];
        postalCoderegExp[0] = /^[0-9]{5}$/;
        postalCoderegExp[1] = /^[0-9]{3}-[0-9]{2}$/;
        postalCoderegExp[2] = /^[0-9]{3}\s[0-9]{2}$/;		//reguljärautryck samlas i en Array
        ...
        var postalCodeField = document.getElementById("postalcodefield");// Tilldelar array id
            
        if (postalCodeField.lastChild.hasChildNodes()) // Tar bort eventuellt utskrivet felmeddelande
        {
            postalCodeField.lastChild.remove(postalCodeField.lastChild);  //ta bort 
        }
        ...
        {
            input.value = input.value.trim();		// om fältet inte är tomt. går igenom Arrayen
            
            var correctFormat = false;							
            for (var i=0; i < postalCoderegExp.length; i++)			// går igenom arrayen med reg.utryck- och jämför med det inmatade i fälten
            {
                if (input.value.match(postalCoderegExp[i]))
                {
                    input.value = input.value.replace(/\s+/g, '');
                    input.value = input.value.replace(/(SE)/, '');		//ersätter textfälten om någpt matchar reg.utrycken
            ...
            if (!correctFormat)			// om det inte är korrekt	
            {    
    ...
    validateEmail: function(input)
    {
        ...
        if (input.value.trim() == "")
        {
            var emptyErrorMessage = document.createTextNode("Detta fält får inte lämnas blankt."); // Är det tomt ?
            var emptyErrorTag = document.createElement("p"); // P-taggen som texten ska stoppas i
            emptyErrorTag.appendChild(emptyErrorMessage);
